...
def build_train_transforms(
    ...
    transform_tr += [c_trans.Resize((height, width))]

    if 'random_flip' in config.transforms:
        print('+ random flip')
        transform_tr += [c_trans.RandomHorizontalFlip()]

    if 'pad' in config.transforms:
        print('+ Pad')
        transform_tr += [c_trans.Pad(10)]

    if 'random crop' in config.transforms:
        print('+ random crop')
        transform_tr += [c_trans.RandomCrop(size=(height, width))]

    print('+ to numpy array of range [0, 1]')
    transform_tr += [c_trans.Rescale(1.0/255.0, 0.0)]

    if norm_mean is None or norm_std is None:
        norm_mean = [0.485, 0.456, 0.406]  # imagenet mean
        norm_std = [0.229, 0.224, 0.225]  # imagenet std
    normalize = c_trans.Normalize(mean=norm_mean, std=norm_std)
    print('+ Normalization (mean={}, std={})'.format(norm_mean, norm_std))
    transform_tr += [normalize]

    print("+ HWC2CHW()")
    transform_tr += [c_trans.HWC2CHW()]

    if 'random_erase' in config.transforms and config.rea:
        print('+ random erase')
